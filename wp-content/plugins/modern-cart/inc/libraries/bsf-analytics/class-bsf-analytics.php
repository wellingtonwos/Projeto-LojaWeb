	...
	class BSF_Analytics {
		...
		public function option_notice() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				if ( is_multisite() ) {
					$notice_string .= __( 'This will be applicable for all sites from the network.', 'modern-cart' );
				}
				...
				Astra_Notices::add_notice(
					array(
						...
						'message'                    => sprintf(
							...
							sprintf( $notice_string . '<span dir="%1s"><a href="%2s" target="_blank" rel="noreferrer noopener">%3s</a><span><br><br>', $language_dir, esc_url( $usage_doc_link ), __( ' Know More.', 'modern-cart' ) ),
							esc_url(
								add_query_arg(
									array(
		...
		public function register_usage_tracking_setting() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				add_settings_field(
					...
					__( 'Usage Tracking', 'modern-cart' ),       // Field title.
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Allow %s products to track non-sensitive usage tracking data.', 'modern-cart' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'modern-cart' );
				}
				?>
			</label>
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $args['usage_doc_link'] ), __( 'Learn More.', 'modern-cart' ) ) );
			?>
			</fieldset>
			<?php
		}
