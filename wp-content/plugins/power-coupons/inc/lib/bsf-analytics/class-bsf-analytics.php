	...
	class BSF_Analytics {
		...
		public function option_notice() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$notice_string = sprintf(
					__(
						'Help shape the future of %1$s!<br><br>Become a contributor by sharing how you use %1$s — so we can build features that matter to you, fix issues faster, and make smarter decisions. We only collect <strong>non-sensitive</strong> information like your PHP version and which features you use.', 'power-coupons' ),
					'<strong>' . esc_html( $data['product_name'] ) . '</strong>'
				);
				
				if ( is_multisite() ) {
					$notice_string .= __( 'This will be applicable for all sites from the network.', 'power-coupons' );
				}
				...
				Astra_Notices::add_notice(
					array(
						...
						'message'                    => sprintf(
							...
							sprintf( $notice_string . '<span dir="%1s"> <a href="%2s" target="_blank" rel="noreferrer noopener">%3s</a><span><br><br>', $language_dir, esc_url( $usage_doc_link ), __( 'Learn more.', 'power-coupons' ) ),
							esc_url(
								add_query_arg(
									array(
							...
							__( 'Sure, count me in!', 'power-coupons' ),
							esc_url(
								add_query_arg(
									array(
		...
		public function register_usage_tracking_setting() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				add_settings_field(
					...
					__( 'Usage Tracking', 'power-coupons' ),       // Field title.
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Help improve %s by sharing non-sensitive usage data — like PHP version and features used.', 'power-coupons' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'power-coupons' );
				}
				?>
			</label>
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $args['usage_doc_link'] ), __( 'Learn More.', 'power-coupons' ) ) );
			?>
			</fieldset>
			<?php
		}
