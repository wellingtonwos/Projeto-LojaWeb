...
( function ( $ ) {
	const CartFlowsOnboarding = {
		...
		_bind() {
			...
			$( document )
				.on(
					'click',
					'.wcf-start-setup',
					CartFlowsOnboarding._redirect_next_step
				)
				.on(
					'click',
					'.install-page-builder-plugins',
					CartFlowsOnboarding._install_page_builder_plugin
				)
				.on(
					'click',
					'.wcf-import-global-flow',
					CartFlowsOnboarding._import_store_checkout_template
				)
				.on(
					'click',
					'.install-required-plugins',
					CartFlowsOnboarding._install_required_plugins
				)
				.on(
					'wp-plugin-installing',
					CartFlowsOnboarding._pluginInstalling
				)
				.on(
					'wp-plugin-install-error',
					CartFlowsOnboarding._installError
				)
				.on(
					'wp-plugin-install-success',
					CartFlowsOnboarding._installSuccess
				);
		},
		 ...
		 */
		_redirect_next_step() {
			// Redirect to Page builder step from home page.
		...
		_install_page_builder_plugin( event ) {
			...
			const plugin_key =
				...
				plugin_data = cartflows_onboarding.page_builders[ plugin_key ],
			...
			if ( 'yes' === is_installed && 'no' === is_activeted ) {
				CartFlowsOnboarding._activatePlugin(
					plugin_init,
					plugin_slug,
					true
				);
			} else if ( 'no' === is_installed ) {
				CartFlowsOnboarding._installPlugin( plugin_slug );
			} else {
		...
		_install_required_plugins( event ) {
			...
			if ( CartFlowsOnboarding.remaining_install_plugins ) {
				CartFlowsOnboarding._install_all_plugins();
			} else if ( CartFlowsOnboarding.remaining_active_plugins ) {
				CartFlowsOnboarding._activate_all_plugins();
			} else if (
				! CartFlowsOnboarding.remaining_active_plugins &&
				! CartFlowsOnboarding.remaining_install_plugins
			) {
		...
		_import_store_checkout_template( event ) {
			...
			$.ajax( {
				...
				data: {
					...
					security: cartflows_onboarding.import_store_checkout_nonce,
				...
				.fail( function () {
					...
					has_error.detail.errorMsg =
						cartflows_onboarding.template_import_errors.api.title;
					has_error.detail.callToAction =
						cartflows_onboarding.template_import_errors.api.msg;
					document.dispatchEvent( has_error );
				} );
		},
		...
		_installSuccess( event, args ) {
			...
			setTimeout( function () {
				CartFlowsOnboarding._activatePlugin( plugin_init, plugin_slug );

				if ( CartFlowsOnboarding.remaining_install_plugins > 0 ) {
					CartFlowsOnboarding.remaining_install_plugins--;
				}
		...
		_install_all_plugins() {
			...
			$.each( required_plugins, function ( index, plugin ) {
				...
				if ( is_checked && 'not-installed' === status ) {
					CartFlowsOnboarding._installPlugin( pluginSlug );
				}
			} );
		},
	...
	function save_page_builder_option( plugin_slug ) {
		...
		$.ajax( {
			...
			data: {
				...
				security: cartflows_onboarding.page_builder_save_option_nonce,
