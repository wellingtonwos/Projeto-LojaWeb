...
$filter = isset( $_GET['filter'] ) ? sanitize_key( $_GET['filter'] ) : 'plugins';

// Select correct database row
switch ( $filter ) {
	case 'themes':
		$db_table 		= 'notUpdateListTh';
		$filter_name 	= esc_html__( 'Themes', 'companion-auto-update' );
		$filterFunction = wp_get_themes();
		break;
	case 'plugins':
		$db_table 		= 'notUpdateList';
		$filter_name 	= esc_html__( 'Plugins', 'companion-auto-update' );
...
<ul class="subsubsub">
	<li><a <?php if( $filter == 'plugins' ) { echo "class='current'"; } ?> href='<?php echo cau_url( 'pluginlist&filter=plugins' ); ?>'><?php esc_html_e( 'Plugins', 'companion-auto-update' ); ?></a></li> |
	<li><a <?php if( $filter == 'themes' ) { echo "class='current'"; } ?> href='<?php echo cau_url( 'pluginlist&filter=themes' ); ?>'><?php esc_html_e( 'Themes', 'companion-auto-update' ); ?></a></li>
</ul>
...
<p>
	...
	echo sprintf( esc_html__( 'Prevent certain %1$s from updating automatically. %2$s that you select here will be skipped by Companion Auto Update and will require manual updating.', 'companion-auto-update' ), strtolower( $filter_name ), $filter_name ); 
	?>
</p>
...
if( isset( $_POST['submit'] ) ) {
	...
	$success = esc_html__( 'Succes', 'companion-auto-update' );
	/* translators: number of items and update type (like 'plugins' or 'themes') */
	$message = sprintf( esc_html__( '%1$s %2$s have been added to the no-update-list', 'companion-auto-update' ), $noUpdateCount, strtolower( $filter_name ) );
	echo '<div id="message" class="updated"><p><b>'.esc_attr( $success ).' &ndash;</b> '.esc_attr( $message ).'.</p></div>';
}
...
if( isset( $_POST['reset'] ) ) {
	...
	$success = esc_html__( 'Succes', 'companion-auto-update' );
	/* translators: update type, like 'plugins' or 'themes' */
	$message = sprintf( esc_html__( 'The no-update-list has been reset, all %s will be auto-updated from now on', 'companion-auto-update' ), strtolower( $filter_name ) );
	echo '<div id="message" class="updated"><p><b>'.esc_attr( $success ).' &ndash;</b> '.esc_attr( $message ).'.</p></div>';
}
...
<form method="POST">
	...
	<div class='pluginListButtons'>
		<?php submit_button(); ?>
		<input type='submit' name='reset' id='reset' class='button button-alt' value='<?php esc_html_e( "Reset list", "companion-auto-update" ); ?>'>
	</div>
	...
	<table class="wp-list-table widefat autoupdate striped">
		<thead>
			<tr>
				<td>&nbsp;</td>
				<th class="head-plugin"><strong><?php esc_html_e( 'Name', 'companion-auto-update' ); ?></strong></th>
				<th class="head-status"><strong><?php esc_html_e( 'Status', 'companion-auto-update' ); ?></strong></th>
				<th class="head-description"><strong><?php esc_html_e( 'Description', 'companion-auto-update' ); ?></strong></th>
			</tr>
		</thead>
